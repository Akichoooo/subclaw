...
function Render-Snapshot {
  ...
  Write-Output "=== subclaw / claw-proxy status (kimi branch) ==="
  ...
  if ($ReportsDir -and (Test-Path -LiteralPath $ReportsDir)) {
    ...
    $reports = Get-ChildItem -LiteralPath $ReportsDir -Include "*.claw.*.md","*.codexclaw.*.md","*.kimiclaw.*.md" -File -Recurse -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First $Recent
    foreach ($file in $reports) {
      $exitLine = Select-String -LiteralPath $file.FullName -Pattern '^\[EXIT\]' -Encoding UTF8 -ErrorAction SilentlyContinue | Select-Object -Last 1
      $state = if ($exitLine) { $exitLine.Line } else { "running/no-exit-yet" }
      $markerPrefix = '^\s*(?:[#*-]+\s*)?'
      $progress = Select-String -LiteralPath $file.FullName -Pattern "$markerPrefix\[PROGRESS\]" -Encoding UTF8 -ErrorAction SilentlyContinue | Select-Object -Last 1
      $asks = @(Select-String -LiteralPath $file.FullName -Pattern "$markerPrefix\[ASK_ORCHESTRATOR\]" -Encoding UTF8 -ErrorAction SilentlyContinue)
      $claims = @(Select-String -LiteralPath $file.FullName -Pattern "$markerPrefix\[CLAIM\]" -Encoding UTF8 -ErrorAction SilentlyContinue)
      $evidence = @(Select-String -LiteralPath $file.FullName -Pattern "$markerPrefix\[EVIDENCE\]" -Encoding UTF8 -ErrorAction SilentlyContinue)
