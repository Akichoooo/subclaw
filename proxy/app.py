...
每次请求轮询下一个 key，均匀消耗；带缓存命中统计、empty-text-block sanitize、degraded 重试。"""
...
# 2. 将同一 session 强绑定到同一 API Key，最大化利用大模型的 Prompt Cache，极大节省 Token 费用。
# 3. 在匹配该 model 的 primary key 里选活跃 session 最少的，实现并发负载均衡。
...
def _entry_matches_model(entry: Dict, model: str) -> bool:
    """entry 的 models 字段 = 通配; 否则要求 model 命中。"""
...
def routing_for_model(model: str) -> List[int]:
    """返回能服务该 model 的 key 下标, primary 在前 overflow 在后 (失败时按此顺序升级重试)。"""
...
def pick_key_for_session(session_id: str, model: str) -> int:
    ...
    with _session_lock:
        ...
        # 已有 session 且该 key 仍能服务该 model 时复用 (保持缓存)
...
def _session_id_from(request: "Request") -> str:
    """优先用 x-session-id 头 (worker 注入, 稳定); 回退 ip:port (TCP 连接, 易碎)。"""
...
def _anthropic_upstream_headers(key: str, req_headers) -> Dict[str, str]:
    """原生 Anthropic 直传: Bearer 鉴权 + 透传 anthropic-version/beta。"""
    out = {
...
# --- Anthropic 原生直传 (保留 tool_use, 不做 OpenAI 转换) ---
...
async def proxy_anthropic_messages(request: Request):
    ...
    # 原生 Anthropic 直传 (protocol="anthropic"): 转发原始 body, 保留 tool_use / tool_result
...
async def _proxy_anthropic_passthrough(
    ...
    """非流式原生直传 + 429/5xx 换 key 重试。body 原样转发, tool_use 保留。"""
    global _upstream_last_success, _upstream_healthy
...
async def _proxy_anthropic_passthrough_stream(
    ...
    """流式原生直传: 探测首响应状态, 429/5xx 换 key; 成功后原样转发 SSE (保留 tool_use 事件)。"""
    global _upstream_last_success, _upstream_healthy
