...
每次请求轮询下一个 key，均匀消耗。
带缓存命中统计、empty-text-block sanitize、degraded 重试。
"""
...
# 1. session id (x-session-id header) 决定了缓存的亲和性 (Cache Affinity).
# 2. 将同一个 session 强绑定到同一个 API Key，最大化利用大模型的 Prompt Cache，极大节省 Token 费用。
# 3. 新 session → 在匹配该 model 的 primary key 里选活跃最少的，实现并发负载均衡。
# 4. 如果触发 429 限流，自动漂移到下一个可用 Key (rebind_session)。
# ============================================
_session_map: Dict[str, int] = {}
_key_sessions: Dict[int, set] = {}
_session_ttl: Dict[str, float] = {}
_session_lock = threading.Lock()
_SESSION_TIMEOUT = 600  # 10 分钟无活动自动释放
_next_key_idx = 0


def _entry_matches_model(entry: Dict, model: str) -> bool:
    """entry 无 models 字段 = 通配; 否则要求 model 命中。"""
...
def routing_for_model(model: str) -> List[int]:
    """返回能服务该 model 的 key 下标, primary 在前 overflow 在后 (失败时按此顺序升级)。"""
...
def pick_key_for_session(session_id: str, model: str) -> int:
    """同一 session_id 固定同一 key; 新 session 在匹配 model 的 primary key 里选负载最轻的。返回 key 下标。"""
    global _next_key_idx
    now = time.time()
    with _session_lock:
        _release_stale(now)

        # 已有 session 且该 key 仍能服务此 model → 复用 (保缓存)
...
def rebind_session(session_id: str, new_idx: int):
    """429 失败转移: 把 session 重新钉到新 key 上 (后续请求保持在新 key 以维持缓存)。"""
    with _session_lock:
...
# --- Stats (缓存命中/成本统计与熔断记账) ---
# 该模块负责追踪整个系统的 Token 消耗，为 /subclaw 的高并发任务提供数据支撑。
# 通过记录 input/output/cache_read tokens，系统能够精确计算并实现防破产熔断（Circuit Breaker）。
...
def _session_id_from(request: "Request") -> str:
    """优先取 x-session-id 头 (worker 注入, 稳定); 回退 ip:port (TCP 连接, 易碎)。"""
...
def _anthropic_upstream_headers(key: str, req_headers) -> Dict[str, str]:
    """原生 Anthropic 直通: Bearer 鉴权 + 透传 anthropic-version/beta。"""
    out = {
...
# --- Anthropic 原生直通 (保留 tool_use, 不做 OpenAI 转换) ---

async def _passthrough_post_once(
    url: str, payload: Dict, headers: Dict[str, str]
) -> Tuple[int, bytes, Dict[str, str]]:
    """单次 POST, 不重试 (429 由上层换 key 处理)。"""
    try:
        async with httpx.AsyncClient(timeout=_HTTPX_TIMEOUT) as client:
            resp = await client.post(url, json=payload, headers=headers)
...
def _key_order_on_failure(session_id: str, model: str, current_idx: int) -> List[int]:
    """429/5xx 时换 key 的尝试顺序: 当前 key 之后的同 model key (primary 优先)。"""
...
# A second hardcoded /models definition used to live here but was dead code —
# FastAPI uses the FIRST registered route for duplicate paths, which made the
...
async def proxy_anthropic_messages(request: Request):
            ...
            if isinstance(c, list):
                for b in reversed(c):
                    if isinstance(b, dict) and b.get("type") == "tool_use":
                        ...
                        latest_action = f"🛠️ 工具: {b.get('name')} {inp_str}"
                        break
    ...
    # 原生 Anthropic 直通 (protocol="anthropic"): 转发原始 body, 保留 tool_use / tool_result。
    ...
    # 回退: OpenAI 协议上游 (会丢 tool_use, 仅用于纯文本模型)。
    current_key = entry["key"]
    upstream_base = entry["url"]
...
async def _proxy_anthropic_passthrough(
    ...
    """非流式原生直通 + 429/5xx 换 key 重试。body 原样转发, tool_use 保留。"""
    global _upstream_last_success, _upstream_healthy
    ...
    for idx in queue:
        ...
        if status == 429:
            ...
            continue
        # 5xx: 上游服务故障, 所有 key 指向同一上游, 切换无效, 直接返回给客户端
        if status >= 500:
            stats.upstream_5xx += 1
            logger.warning(f"5xx on key[{idx}] (upstream service error, no failover) -> return to client")
            return Response(content=body_bytes, status_code=status, media_type="application/json")
        # status == 0: 连接失败, 可能是网络问题, 尝试下一个 key
        if status == 0:
            stats.upstream_retries += 1
            logger.warning(f"status=0 on key[{idx}] (connect failed) -> failover")
            continue
        # 4xx (非 429): 客户端错误, 换 key 也没用, 直接回传。
        logger.info(f"<- {status} passthrough (client error, no failover) | key=...{entry['key'][-6:]}")
        return Response(content=body_bytes, status_code=status, media_type="application/json")
...
async def _proxy_anthropic_passthrough_stream(
    ...
    """流式原生直通: 探测首响应状态, 429/5xx 换 key; 成功后原样转发 SSE (保留 tool_use 事件)。"""
    global _upstream_last_success, _upstream_healthy
...
async def list_models():
    """自描述: 可用模型 + 每个模型的上游协议/层级。技能文档只引用此接口, 不抄表。"""
    ...
    return JSONResponse(content={
        ...
        "note": "passthrough=true 模型保留 tool_use/tool_result (可跑 Read/Glob/Grep 等工具); false 仅纯文本。",
        "data": data,
    })
